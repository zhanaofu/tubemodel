...
The glottal source is generated with a polynomial model from Rosenberg (1971). No air is assumed to go back through the mouth or the glottis. The transfer function of the filter is calculated with numerical polynomial arithmetic. Tube wall losses are modelled by shifting the filter poles inward, giving each formant a bandwidth of approximately 60 Hz. The FFT transforms are done with the [DSP.js](https://github.com/corbanbrook/dsp.js/) library. The color scale for the spectrograms are generated with the [chroma.js](https://vis4.net/chromajs/) library.
